...
type ColumnType = ColumnDef<WatchlistInterface, unknown>;
...
export function WatchlistListPage(props: WatchlistListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WatchlistInterface>>(
    () => `/watchlists?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WatchlistInterface) => {
    if (hasAccess('watchlist', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/watchlists/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    hasAccess('anime', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'anime',
          header: 'Anime',
          accessorKey: 'anime',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/anime/view/${record.anime?.id}`}>
              {record.anime?.title}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/watchlists/view/${record.id}`} passHref legacyBehavior>
                <Button

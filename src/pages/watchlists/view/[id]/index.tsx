...
import { getWatchlistById } from 'apiSdk/watchlists';
import { WatchlistInterface } from 'interfaces/watchlist';

function WatchlistViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('watchlist', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/watchlists/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('anime', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Anime"
                    text={
                      <Link as={NextLink} href={`/anime/view/${data?.anime?.id}`}>
                        {data?.anime?.title}
                      </Link>
                    }
                  />

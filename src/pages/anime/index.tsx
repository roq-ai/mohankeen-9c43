...
import { getAnime, deleteAnimeById } from 'apiSdk/anime';
import { AnimeInterface } from 'interfaces/anime';

type ColumnType = ColumnDef<AnimeInterface, unknown>;
...
export function AnimeListPage(props: AnimeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getAnime({
        relations: [, 'review.count', 'watchlist.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['title.contains', 'genre.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AnimeInterface>>(
    () => `/anime?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AnimeInterface) => {
    if (hasAccess('anime', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/anime/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'episodes', header: 'episodes', accessorKey: 'episodes' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/anime/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('anime', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/anime/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('anime', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="anime" />
        </Flex>

        {hasAccess('anime', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/anime/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

...
import { getWatchlists } from 'apiSdk/watchlists';
import { WatchlistInterface } from 'interfaces/watchlist';
import { getModerations } from 'apiSdk/moderations';
import { ModerationInterface } from 'interfaces/moderation';
import { getAnime } from 'apiSdk/anime';
import { AnimeInterface } from 'interfaces/anime';

import { ModerationListPage as ShowCaseList } from 'pages/moderations';

function DashboardPage() {
  const { session } = useSession();

  const watchlistStat = useSWR<PaginatedInterface<WatchlistInterface>>(
    () => `/watchlists`,
    () => getWatchlists({ limit: 1, offset: 0 }),
  );

  const moderationStat = useSWR<PaginatedInterface<ModerationInterface>>(
    () => `/moderations`,
    () => getModerations({ limit: 1, offset: 0 }),
  );

  const animeStat = useSWR<PaginatedInterface<AnimeInterface>>(
    () => `/anime`,
    () => getAnime({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Watchlists',
      result: watchlistStat?.data?.totalCount,
      isLoading: watchlistStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Moderations',
      result: moderationStat?.data?.totalCount,
      isLoading: moderationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Anime',
      result: animeStat?.data?.totalCount,
      isLoading: animeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`moderations`} passHref legacyBehavior>
                  <Button
